jQuery(document).ready(function()
{
	var isAllow = ["q","w","e","r","t","y","u","i","o","p","a","s","d","f","g","h","j","k","l","ñ","z","x","c","v","b","n","m"," ","á","é","í","ó","ú","'"];
		...
		var isAllowSerialNumber = ["q","w","e","r","t","y","u","i","o","p","a","s","d","f","g","h","j","k","l","ñ","z","x","c","v","b","n","m","á","é","í","ó","ú","1","2","3","4","5","6","7","8","9","0","-"];
	...
	jQuery('body').on("keyup",'.serial_number_format',function(e)
	{
		if(e.keyCode != 36 && e.keyCode != 16 && e.keyCode != 46 && e.keyCode != 9 && e.keyCode != 39 && e.keyCode != 37)
		{
			var s = jQuery(this).val();
			...
			for(i = 0; i < s.length; i++)
			{
				c = s.charAt(i);
				c = c.toLowerCase();
				if(jQuery.inArray(c,isAllowSerialNumber) == -1)
				{
					value = value.replace(s.charAt(i),'');
					jQuery(this).val(value);
				}
	...
	jQuery('body').on("keypress",".serial_number_format",function()
	{
		var s = jQuery(this).val();
		value = s;
		for(i = 0; i < s.length; i++)
	...
	jQuery('body').on("blur",".serial_number_format",function()
	{
		var s = jQuery(this).val();
		value = s;
		for(i = 0; i < s.length; i++)
	...
	jQuery(".serial_number_format").bind("blur",function()
	{
		jQuery(this).val(remove_space_all(jQuery(this).val()));
	});
});